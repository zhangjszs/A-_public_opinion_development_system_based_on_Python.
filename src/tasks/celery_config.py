...
celery_app = Celery(
    'weibo_sentiment',
    broker=Config.REDIS_URL if hasattr(Config, 'REDIS_URL') else 'redis://localhost:6379/0',
    backend=Config.REDIS_URL if hasattr(Config, 'REDIS_URL') else 'redis://localhost:6379/0',
    include=[
