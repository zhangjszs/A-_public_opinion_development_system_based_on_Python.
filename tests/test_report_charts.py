...
def test_pdf_generate_with_charts_returns_path(pdf_gen, sample_data):
    ...
    with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as f:
        out = f.name
    try:
        result = pdf_gen.generate(sample_data, output_path=out, config=ReportConfig(include_charts=True))
        assert result == out
...
def test_pdf_with_charts_larger_than_without(pdf_gen, sample_data):
    ...
    with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as f:
        out_without = f.name
    try:
        pdf_gen.generate(sample_data, output_path=out_with, config=ReportConfig(include_charts=True))
        pdf_gen.generate(sample_data, output_path=out_without, config=ReportConfig(include_charts=False))
...
def test_pdf_generate_without_charts_still_works(pdf_gen, sample_data):
    ...
    with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as f:
        out = f.name
    try:
        result = pdf_gen.generate(sample_data, output_path=out, config=ReportConfig(include_charts=False))
        assert result == out
        assert os.path.exists(out)
    finally:
        if os.path.exists(out):
...
def test_pdf_charts_skipped_when_no_trend(pdf_gen):
    ...
    data = {'summary': {'positive_count': 10, 'neutral_count': 5, 'negative_count': 2,
                        'total_articles': 17, 'total_comments': 30}}
    with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as f:
        out = f.name
    try:
        result = pdf_gen.generate(data, output_path=out, config=ReportConfig(include_charts=True))
        assert result == out
    finally:
        if os.path.exists(out):
            os.unlink(out)
...
def test_ppt_generate_with_charts_returns_path(ppt_gen, sample_data):
    ...
    with tempfile.NamedTemporaryFile(suffix='.pptx', delete=False) as f:
        out = f.name
    try:
        result = ppt_gen.generate(sample_data, output_path=out, config=ReportConfig(include_charts=True))
        assert result == out
        assert os.path.exists(out)
        assert os.path.getsize(out) > 0
    finally:
...
def test_ppt_with_charts_larger_than_without(ppt_gen, sample_data):
    ...
    with tempfile.NamedTemporaryFile(suffix='.pptx', delete=False) as f:
        out_without = f.name
    try:
        ppt_gen.generate(sample_data, output_path=out_with, config=ReportConfig(include_charts=True))
        ppt_gen.generate(sample_data, output_path=out_without, config=ReportConfig(include_charts=False))
        assert os.path.getsize(out_with) > os.path.getsize(out_without)
    finally:
        for p in (out_with, out_without):
            if os.path.exists(p):
...
def test_ppt_generate_without_charts_still_works(ppt_gen, sample_data):
    ...
    with tempfile.NamedTemporaryFile(suffix='.pptx', delete=False) as f:
        out = f.name
    try:
        result = ppt_gen.generate(sample_data, output_path=out, config=ReportConfig(include_charts=False))
        assert result == out
        assert os.path.exists(out)
    finally:
        if os.path.exists(out):
...
def test_ppt_charts_skipped_when_no_alerts(ppt_gen):
    ...
    with tempfile.NamedTemporaryFile(suffix='.pptx', delete=False) as f:
        out = f.name
    try:
        result = ppt_gen.generate(data, output_path=out, config=ReportConfig(include_charts=True))
        assert result == out
    finally:
        if os.path.exists(out):
            os.unlink(out)
