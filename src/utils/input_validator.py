...
class InputValidator:
    ...
    def validate_username(username: str) -> dict:
        ...
        if len(username) < InputValidator.USERNAME_MIN_LENGTH:
            return {'valid': False, 'message': f'用户名长度至少{InputValidator.USERNAME_MIN_LENGTH}位'}
        
        if len(username) > InputValidator.USERNAME_MAX_LENGTH:
            return {'valid': False, 'message': f'用户名长度最多{InputValidator.USERNAME_MAX_LENGTH}位'}
    ...
    def validate_password(password: str) -> dict:
        ...
        if len(password) < InputValidator.PASSWORD_MIN_LENGTH:
            return {'valid': False, 'message': f'密码长度至少{InputValidator.PASSWORD_MIN_LENGTH}位'}
        
        if len(password) > InputValidator.PASSWORD_MAX_LENGTH:
            return {'valid': False, 'message': f'密码长度最多{InputValidator.PASSWORD_MAX_LENGTH}位'}
    ...
    def validate_keyword(keyword: str) -> dict:
        ...
        if len(keyword) > InputValidator.KEYWORD_MAX_LENGTH:
            return {'valid': False, 'message': f'关键词长度最多{InputValidator.KEYWORD_MAX_LENGTH}位'}
    ...
    def sanitize_sql(text: str) -> str:
        ...
        sanitized = re.sub(r'(?i)(union|select|insert|update|delete|drop|alter|create|truncate|exec|execute)', '', sanitized)
