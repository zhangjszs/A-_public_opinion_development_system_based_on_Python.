...
class PinyinConverter:
    ...
    def _init_pinyin_map(self) -> Dict[str, str]:
        """初始化拼音映射表"""
        return {
            '微': 'wei', '博': 'bo', '舆': 'yu', '情': 'qing', '分': 'fen', '析': 'xi',
            '系': 'xi', '统': 'tong', '测': 'ce', '试': 'shi', '用': 'yong', '户': 'hu',
            '评': 'ping', '论': 'lun', '关': 'guan', '键': 'jian', '词': 'ci', '搜': 'sou',
            '索': 'suo', '数': 'shu', '据': 'ju', '展': 'zhan', '示': 'shi', '图': 'tu',
            '表': 'biao', '热': 're', '点': 'dian', '话': 'hua', '题': 'ti', '新': 'xin',
            '闻': 'wen', '报': 'bao', '道': 'dao', '转': 'zhuan', '发': 'fa', '点': 'dian',
            '赞': 'zan', '正': 'zheng', '负': 'fu', '面': 'mian', '中': 'zhong', '性': 'xing',
            '北': 'bei', '京': 'jing', '上': 'shang', '海': 'hai', '广': 'guang', '州': 'zhou',
            '深': 'shen', '圳': 'zhen', '杭': 'hang', '州': 'zhou', '南': 'nan', '东': 'dong',
            '西': 'xi', '好': 'hao', '坏': 'huai', '是': 'shi', '不': 'bu', '有': 'you',
            '无': 'wu', '大': 'da', '小': 'xiao', '多': 'duo', '少': 'shao', '高': 'gao',
            '低': 'di', '快': 'kuai', '慢': 'man', '人': 'ren', '事': 'shi', '物': 'wu',
        }
...
class AdvancedSearchEngine:
    ...
    def __init__(self, db_path: str = None):
        self.db_path = db_path or str(Path(__file__).parent.parent.parent / 'data' / 'search.db')
    ...
    def index_document(self, doc_id: str, title: str, content: str,
                       source_type: str = 'article', author: str = '',
                       created_at: str = None):
        """索引文档"""
        ...
        with self._lock:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (doc_id, title, content, source_type, author, created_at, pinyin_title, pinyin_content))
    ...
    def search(self, query: str, limit: int = 20, offset: int = 0,
               source_type: str = None, order_by: str = 'relevance') -> List[SearchResult]:
        """搜索"""
        results = []
    ...
    def _fallback_search(self, query: str, limit: int, offset: int,
                         source_type: str = None) -> List[SearchResult]:
        """回退搜索（LIKE）"""
        results = []
        like_query = f'%{query}%'
    ...
    def _extract_highlights(self, content: str, query: str, max_length: int = 100) -> List[str]:
        """提取高亮片段"""
        highlights = []
        words = query.split()
    ...
    def search_by_pinyin(self, pinyin_query: str, limit: int = 20) -> List[SearchResult]:
        """拼音搜索"""
        results = []
    ...
    def get_stats(self) -> Dict:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute('SELECT COUNT(DISTINCT source_type) FROM search_index')
